...
// Parse received data packet and return result
...
// Control servo to move to a position over a specified time
...
int LobotSerialServoReadID(HardwareSerial &SerialX)
{
  ...
  buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
  buf[2] = ID_ALL;  // ID_ALL is 254, which broadcasts to all servos; useful for discovering unknown IDs
  buf[3] = 3;
...
// Read rotation range (min and max angles)
int retL;
int retH;
...
// Read voltage range (min and max VIN)
int vinL;
int vinH;
...
// Read maximum temperature threshold
...
// Read whether servo is loaded or unloaded
